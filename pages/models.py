...
class HomePageContent(models.Model):
    ...
    about_image = models.ImageField(
        upload_to="pages/home/", verbose_name=_("Изображение"), blank=True, null=True
    )
    ...
    advantages = models.JSONField(
        ...
        default=list,
        help_text=_(
            'Список преимуществ в формате [{"title": "...", "description": "..."}]'
        ),
    )
    ...
    doctors_count = models.PositiveIntegerField(
        verbose_name=_("Количество врачей"), default=0
    )
    experience_years = models.PositiveIntegerField(
        verbose_name=_("Лет на рынке"), default=0
    )

    # SEO
    meta_title = models.CharField(
        max_length=200, verbose_name=_("Meta Title"), blank=True
    )
    meta_description = models.TextField(
        max_length=500, verbose_name=_("Meta Description"), blank=True
    )
    ...
    def clean(self):
        if not self.pk and HomePageContent.objects.exists():
            raise ValidationError(
                "Можно создать только одну запись контента главной страницы"
            )
...
class AboutPageContent(models.Model):
    ...
    title = models.CharField(
        max_length=200, verbose_name=_("Заголовок страницы"), default="О компании"
    )
    ...
    history_text = models.TextField(verbose_name=_("Текст истории компании"))
    ...
    mission_title = models.CharField(
        max_length=200, verbose_name=_("Заголовок миссии"), default="Наша миссия"
    )
    mission_text = models.TextField(verbose_name=_("Текст миссии"))

    # Ценности
    values_title = models.CharField(
        max_length=200, verbose_name=_("Заголовок ценностей"), default="Наши ценности"
    )
    values = models.JSONField(
        verbose_name=_("Ценности"),
        default=list,
        help_text=_(
            'Список ценностей в формате [{"title": "...", "description": "..."}]'
        ),
    )
    ...
    equipment_description = models.TextField(
        verbose_name=_("Описание оборудования"), blank=True
    )
    ...
    def clean(self):
        if not self.pk and AboutPageContent.objects.exists():
            raise ValidationError(
                "Можно создать только одну запись контента страницы 'О компании'"
            )
