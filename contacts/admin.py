...
class FeedbackMessageAdmin(admin.ModelAdmin):
    ...
    """
    list_display = (
        'name', 'email', 'phone', 'short_message',
        'created_at', 'is_processed', 'process_button'
    )
    ...
    def process_button(self, obj):
        ...
        if not obj.is_processed:
            ...
            return format_html(
                '<a class="button" href="{}" style="background-color: #28a745; color: white; padding: 3px 10px; border-radius: 3px; text-decoration: none;">Отметить обработанным</a>',
                url
            )
    ...
    def mark_as_processed(self, request, queryset):
        ...
        self.message_user(request, f"Отмечено {queryset.count()} сообщений как обработанные")
    ...
    def mark_as_unprocessed(self, request, queryset):
        ...
        self.message_user(request, f"У {queryset.count()} сообщений снята отметка об обработке")
