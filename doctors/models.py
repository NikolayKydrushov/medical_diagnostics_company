...
class Doctor(models.Model):
    ...
    name = models.CharField(max_length=100, verbose_name=_("ФИО врача"), db_index=True)
    slug = models.SlugField(
        max_length=250, unique=True, verbose_name=_("URL-идентификатор"), blank=True
    )
    ...
    photo = models.ImageField(
        upload_to="doctors/", verbose_name=_("Фото"), blank=True, null=True
    )
    bio = models.TextField(
        verbose_name=_("Биография"), help_text=_("Образование, опыт, достижения")
    )
    experience_years = models.PositiveIntegerField(
        verbose_name=_("Стаж работы (лет)"), default=0
    )
