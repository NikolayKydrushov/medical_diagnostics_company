...
class AppointmentAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        (None, {"fields": ("user", "service", "doctor")}),
        (
            _("Время приема"),
            {
                "fields": ("date", "time"),
    ...
    def mark_as_completed(self, request, queryset):
        ...
        self.message_user(
            request, f"{queryset.count()} записей отмечены как завершенные"
        )
...
class DiagnosticResultAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        (None, {"fields": ("appointment", "result_file")}),
        (
            _("Комментарий"),
            {
    ...
    def appointment_link(self, obj):
        """Ссылка на запись."""
        url = reverse(
            "admin:appointments_appointment_change", args=[obj.appointment.pk]
        )
    ...
    def file_link(self, obj):
        ...
        if obj.result_file:
            return format_html(
                '<a href="{}" target="_blank">📄 Скачать файл</a>', obj.result_file.url
            )
