...
class Appointment(models.Model):
    ...
    time = models.TimeField(verbose_name=_("Время приема"))
    ...
    created_at = models.DateTimeField(
        auto_now_add=True, verbose_name=_("Дата создания записи")
    )
    ...
    def can_cancel(self):
        ...
        if self.status in [self.Status.PENDING, self.Status.CONFIRMED]:
            ...
            return (
                timezone.make_aware(appointment_datetime) - timezone.now()
            ).total_seconds() > 7200
        return False
...
class DiagnosticResult(models.Model):
    ...
    doctor_comment = models.TextField(verbose_name=_("Заключение врача"), blank=True)
    ...
    def clean(self):
        ...
        if self.appointment.status != Appointment.Status.COMPLETED:
            ...
            raise ValidationError(
                "Результаты можно добавить только к завершенным записям"
            )
